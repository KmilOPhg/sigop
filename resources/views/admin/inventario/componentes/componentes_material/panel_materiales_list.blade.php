{{-- Tabla + paginación + modales editar (misma página actual); se reemplaza vía AJAX al paginar. --}}
        ...
        <table class="w-full text-left border-collapse">
            ...
            @forelse ($materiales as $material)
                <tr class="hover:bg-surface-container-lowest transition-colors {{ $material->estado === 'inactivo' ? 'opacity-50' : '' }}">
                    ...
                    <td class="px-6 py-5">
                        <button type="button" class="toggleEstadoBtn px-2 py-1 text-[10px] font-bold rounded {{ $material->estado === 'activo' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}"
                    ...
                    <td class="px-6 py-5 text-right">
                        <button type="button" @click="editId = {{ $material->id }}; showEdit = true"
            ...
            <div class="flex min-h-full items-center justify-center p-8">
                <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg z-10" x-transition.scale.95>
                    <div class="px-6 py-4 border-b border-slate-200/50 flex items-center justify-between">
                        ...
                        <button type="button" @click="showEdit = false" class="p-1 text-slate-400 hover:text-slate-600"><span class="material-symbols-outlined">close</span></button>
                    </div>
