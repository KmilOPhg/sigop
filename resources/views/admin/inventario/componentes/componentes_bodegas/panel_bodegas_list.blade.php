{{-- Tabla + paginación + modales editar; se reemplaza vía AJAX al paginar. --}}
        ...
        <table class="w-full text-left border-collapse">
            ...
            @forelse ($bodegas as $bodega)
                <tr class="hover:bg-surface-container-lowest transition-colors {{ $bodega->estado === 'inactivo' ? 'opacity-50' : '' }}">
                    ...
                    <td class="px-6 py-5">
                        <button type="button" class="toggleEstadoBtn px-2 py-1 text-[10px] font-bold rounded {{ $bodega->estado === 'activo' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}"
                                data-id="{{ $bodega->id }}" data-estado="{{ $bodega->estado === 'activo' ? 'inactivo' : 'activo' }}">
                            {{ ucfirst($bodega->estado) }}
                        </button>
                    </td>
                    <td class="px-6 py-5 text-right">
                        <button type="button" @click="editId = {{ $bodega->id }}; showEdit = true"
