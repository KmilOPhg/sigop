...
@section('content')
    <div class="max-w-lg mx-auto space-y-6">
        <div>
            ...
            <h2 class="text-2xl font-bold tracking-tight text-on-surface">Crear Material</h2>
            <p class="text-xs text-slate-500 mt-1">Registra un nuevo material en el sistema</p>
        </div>
        ...
        <div class="bg-surface-container-low rounded-xl p-6">
            <form method="POST" action="{{ route('admin.materiales.crear.post') }}" class="space-y-5">
                @csrf

                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Item</label>
                    <input type="text" name="item" required
                           class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all @error('item') border-red-400 @enderror">
                    @error('item')
                ...
                <div>
                    ...
                    <input type="text" name="nombre_material" placeholder="Ej. Cordón 120cm" required
